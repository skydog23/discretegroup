...
public class GroupGeneratorFactory {
	...
	static final Logger logger = LoggingSystem.getLogger(GroupGeneratorFactory.class);
	static {
		try {
			logger.setLevel(Level.FINE);
			System.err.println("Setting logging level to "+logger.getLevel().toString());
			logger.info("Here is an fine string GGF");
		} catch(SecurityException se)	{
			System.err.println("Unable to set logger level GGF");;
		}
...
//		gens[3].setWord(generateUniqueWord());
	...
	private static DiscreteGroupElement[] mergeGenerators(DiscreteGroupElement[] gens1, DiscreteGroupElement[] gens2) {
		...
		for (int i = 0; i < ret.length; i++) logger.info(ret[i].getWord());
		logGens(ret);
		return ret;
	}
	/**
...
//						ret[2] = (DiscreteGroupElement) D4.getElement("-(03)(12)");
	...
	private static void logGens(DiscreteGroupElement[] elems) {
		System.err.println("Array of "+elems.length+" group elements.");
		for (int j = 0; j < elems.length; j++)  {
			System.err.println(elems[j].getWord());
			System.err.println(Rn.matrixToString(elems[j].getArray()));
		}
		System.err.println("===");
	}
	...
	public static  DiscreteGroup getD8Group(String name)	{
		...
		dg.setConstraint(masterConstraint);
		return dg;
	}
	
	public static IndexedFaceSet getTetra()	{
		IndexedFaceSetFactory ifsf = new IndexedFaceSetFactory();
		ifsf.setVertexCount(4);
		ifsf.setVertexCoordinates(new double[][] {D4.q0,D4.q1,D4.q2,D4.q3});
		ifsf.setFaceCount(4);
		ifsf.setFaceIndices(new int[][] {{0,1,2},{1,0,3},{1,2,3},{0,2,3}});
		ifsf.setGenerateEdgesFromFaces(true);
		ifsf.setGenerateFaceNormals(true);
		ifsf.setFaceColors(new Color[] {Color.red, Color.blue, Color.green, Color.yellow});
		ifsf.update();
		return(ifsf.getIndexedFaceSet());
	}

	public static void main(String[] args) {
		DiscreteGroup dg = getD8Group("8.");
		dg.setConstraint(new DiscreteGroupSimpleConstraint(20));
		dg.update();
		System.err.println("8. group =\n"+dg.getName()+"num el= "+dg.getElementList().length);
		for (int i = 0; i< dg.getElementList().length; ++i)	{
			System.err.println(i+" = \n"+Rn.matrixToString(dg.getElementList()[i].getArray()));
		}
	}
}
